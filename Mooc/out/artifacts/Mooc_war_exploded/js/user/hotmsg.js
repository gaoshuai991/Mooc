...
function getMsg() {
    if ($("#loadmore").val() == "我也是有底线的...")
        return;
    $.post(getContextPath() + "/user/message!appendHotmsg.action",
        {"curPage": curPage, "pageNumber": pageNumber}, function (data) {
            ...
            for (var x = 0; x < data.length; x++) {
                var ele = $('<div class="col-sm-12 msgitem" style="display: block;">' +
                    '            <h3><a class="msg-title" href="user/message!msg_show.action?msgid=' + data[x].msgid + '">' + data[x].msgtopic + '</a></h3>' +
                    ...
                    '                <a class="msglink" href="user/message!msg_show.action?msgid=' + data[x].msgid + '">详细&gt;&gt;</a>' +
                    '            </div>' +
                    '        </div>');
                $("#msgDiv").append(ele);
            }
